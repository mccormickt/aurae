...
pub(super) async fn get_link_index(
    ...
    match link {
        ...
        Ok(None) => Err(NetworkError::DeviceNotFound { iface }),
        // The kernel answers a lookup for an absent device with a NACK that
        // carries -ENODEV. It does not return an empty stream. Report the
        // absence as `DeviceNotFound`, because [`wait_for_link`] polls on
        // that variant. A hard error there stops the poll, and the endpoint
        // of a cell fails if the parent has not moved the peer into the
        // netns yet.
