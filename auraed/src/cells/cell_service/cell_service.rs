...
impl CellObserveService {
    fn stop_on_shutdown<T, S>(&self, stream: S) -> ObserveStream<T>
    where
        T: 'static,
        S: Stream<Item = std::result::Result<T, Status>> + Send + 'static,
    {
        let mut shutdown = self.observe_service.subscribe_log_stream_shutdown();
        Box::pin(futures::StreamExt::take_until(stream, async move {
...
impl observe_service_server::ObserveService for CellObserveService {
    ...
    > {
        ...
        if let Some(cell_name) = cell_name {
            let request = request.into_inner();
            let mut shutdown =
                self.observe_service.subscribe_log_stream_shutdown();
            let response = tokio::select! {
                _ = shutdown.wait_for(|shutdown| *shutdown) => {
                    return Err(Status::unavailable("auraed is shutting down"));
                }
                response = async {
                    do_in_cell!(
                        self,
            ...
            Ok(Response::new(self.stop_on_shutdown(response.into_inner())))
        } else {
            let response = self
                .observe_service
                .get_aurae_daemon_log_stream(request)
                .await?;
            Ok(Response::new(Box::pin(response.into_inner())))
        }
    }

    type GetSubProcessStreamStream = ObserveStream<GetSubProcessStreamResponse>;
...
mod tests {
    ...
    #[tokio::test]
    async fn observe_proxy_shutdown_ends_nested_stream() {
        let observe_service =
            ObserveService::new(LogChannel::new("auraed"), (None, None, None));
        let proxy = CellService::new(observe_service.clone()).observe_proxy();
        let mut stream = proxy.stop_on_shutdown(tokio_stream::pending::<
            std::result::Result<GetAuraeDaemonLogStreamResponse, Status>,
        >());

        observe_service.shutdown_log_streams();

        let item = tokio::time::timeout(Duration::from_secs(2), stream.next())
            .await
            .expect("stream did not stop before timeout");
        assert!(item.is_none(), "stream should end without an error item");
    }
}
