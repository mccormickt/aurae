...
pub async fn run(
    ...
    // The single LogChannel that backs both the broadcast tracing layer
    // (producer) and ObserveService::get_aurae_daemon_log_stream (consumer).
    // Construct it before init::init so logging::init can register the layer.
