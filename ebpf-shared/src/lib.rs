...
mod tests {
    ...
    fn off_prefix_source_is_rejected_even_for_multicast() {
        ...
        let net = addr(SELF_NET);
        assert!(!cell_source_allowed(&addr(SIBLING), &addr("ff02::1"), &net, 128));
        assert!(!cell_source_allowed(&addr(SIBLING), &addr("ff02::16"), &net, 128));
        assert!(!cell_source_allowed(&addr("fd00:ae::1"), &addr("ff02::2"), &net, 128));
        assert!(!cell_source_allowed(&addr(SIBLING), &addr("2001:db8::1"), &net, 128));
    }
    ...
    fn unspecified_source_is_allowed_only_for_multicast() {
        ...
        assert!(!cell_source_allowed(&addr("::"), &addr("fd00:ae::1"), &net, 128));
    }
    ...
    fn wider_delegation_admits_its_whole_prefix() {
        ...
        assert!(cell_source_allowed(&addr("fd00:ae:0:0:1:dead::5"), &addr(SIBLING), &net, 80));
        assert!(!cell_source_allowed(&addr("fd00:ae:0:0:2::5"), &addr(SIBLING), &net, 80));
    }
